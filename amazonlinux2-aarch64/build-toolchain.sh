...
if [[ "x$linux_swift_pkg" != "x" ]]; then
  ...
  mkdir "$tmp/usr" 
  tar -C "$tmp/usr" --strip-components 1 -xf "$linux_swift_pkg"
...
fix_glibc_modulemap "${BUILD_DIR}/${CROSS_TOOLCHAIN_NAME}/$xc_tc_name/usr/lib/swift/linux/$TARGET_ARCH/glibc.modulemap"
fix_glibc_modulemap "${BUILD_DIR}/${CROSS_TOOLCHAIN_NAME}/$linux_sdk_name/usr/lib/swift/linux/$TARGET_ARCH/glibc.modulemap"
...
cat > "${BUILD_DIR}/${CROSS_TOOLCHAIN_NAME}/destination.json" <<EOF
{
    ...
    "target": "$TARGET_ARCH-unknown-linux",
    "extra-cc-flags": [
        "-fPIC"
    ],
    "extra-swiftc-flags": [
